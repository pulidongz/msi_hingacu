...
class CoordinateField(forms.CharField):
    """
    Validates if a value is of the form (y, x) or (lat, lon) in Decimal Degrees
    """
    ...
    def to_python(self, value):
        ...
        print("pre-coordinate", value)
        try: #this part can be improved with regex
            value = value.replace(' ', '') #remove spaces
            value = value.replace(')', '') #remove parenthesis
            value = value.replace('(', '') #remove parenthesis
            lat, lon = value.split(',') #split the tuple
        except Exception:
            error_message = 'Value should be in the form of a (lat, lon) tuple.'
            raise ValidationError(
                error_message,
                code='invalid',
                params={'value': value},
        ...
        try: #test for decimal
            lat = Decimal(lat)
            lon = Decimal(lon)
        except Exception:
            error_message = 'Values are not in Decimal Degrees (DD) format'
            raise ValidationError(
                error_message,
                code='invalid',
                params={'value': value},
        ...
        if lat < -90 or lat > 90:
            raise ValidationError(
                f'Latitude value {lat} is invalid',
                code='invalid',
                params={'value': value},
            )
        if lon < -180 or lon > 180:
            raise ValidationError(
                f'Longitude value {lon} is invalid',
                code='invalid',
                params={'value': value},
            )
        return value
...
class DCRForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field in self.dcr.keys():
            if self.dcr[field]['field_type'] == 'text':
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field in self.rules:
            if field.field_type == 'text':
                ...
                self.fields[field.field_name] = forms.TimeField(required=field.required)
            ...
            elif field.field_type == 'coordinate':
                self.fields[field.field_name] = CoordinateField(required=field.required)
            elif field.field_type == 'model': #needs an update
                app = field['app']
                model = field['model']
                query_field = field['query_field']
                self.fields[field['field_name']] = ModelField(required=field['required'],
                    model=model, app=app, query_field=query_field)
            elif field.field_type == 'key_value': #needs an update
                self.fields[field['field_name']] = forms.CharField(required=field['required'])
                self.key_value_fields[field] = field['value_field']
            elif field.field_type == 'key_look_up': #needs an update
                etlfile = self.etlfile
                ...
                message = f"Missing field_type: {field}"
                print(message)
